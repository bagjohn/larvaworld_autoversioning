    ...
    def check(self):
        ...
        for f in self.followers:
            if any([geometry.Point(f.get_position()).distance(geometry.Point(p))<f.radius for p in targets_pos]):
                ...
                for a in self.agents:
                    a.brain.olfactor.gain = {id: -v for id, v in a.brain.olfactor.gain.items()}
                break
        ...
        for group, score in self.score.items():
            if score >= 20000.0:
                ...
                return True
        self.set_state(f'L:{np.round(self.score["Left"], 1)} vs R:{np.round(self.score["Right"], 1)}')
        return False
    ...
    def check(self):
        ...
        if carrier is None:
            ...
            if self.r_t - dur > 0:
                ...
                return True
        self.set_state(f'L:{np.round(dur - self.l_t, 2)} vs R:{np.round(dur - self.r_t, 2)}')
        return False
