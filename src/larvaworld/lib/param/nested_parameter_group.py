...
class NestedConf(param.Parameterized):
    ...
    def generator(self, mode='Unit'):
        ...
        A.__name__ = f'{self.__name__}{mode}'
        invalid = ['name', 'closed', 'visible', 'selected', 'centered']
        if mode == 'Group':
            from .xy_distro import Larva_Distro, Spatial_Distro

            if not 'pos' in self.param.objects():
                raise ValueError(f'No Group distribution for class {self.__name__}. Change mode to Unit')
            distro = Larva_Distro if 'orientation' in self.param.objects() else Spatial_Distro
            A.param._add_parameter('distribution',
                                   ClassAttr(distro, doc='The spatial distribution of the group agents'))
            invalid += ['unique_id', 'pos', 'orientation']
        elif mode == 'Unit':
            pass
        for k, p in self.param.params().items():
            if k not in invalid:
